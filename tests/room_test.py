...
class TestRoom(unittest.TestCase):
    def setUp(self):
        ...
        self.room_1.adding_song_to_playlist(self.song_1)
        self.room_1.adding_song_to_playlist(self.song_2)
        ...
        self.room_1.check_in_guest_to_room(self.guest_2)
        self.room_1.check_in_guest_to_room(self.guest_3)
    ...
    def test_adding_guest_to_karaoke_room(self):
        guest_4 = Guest("Sandy", 18, 30.50)
        self.room_1.check_in_guest_to_room(guest_4)
        self.assertEqual(4, len(self.room_1.guest_list))

    def test_removing_guest_from_karaoke_room(self):
        self.room_1.check_out_guest_from_room(self.guest_2)
        self.assertEqual(2, len(self.room_1.guest_list))

    def test_adding_song_to_playlist(self):
        song_3 = Song("Look at Me, I'm Sandra Dee", "Stockard Channing")
        self.room_1.adding_song_to_playlist(song_3)
        self.assertEqual(3, len(self.room_1.song_list))

    def test_karaoke_room_capacity(self):
        self.assertEqual(4, self.room_1.room_capacity)
    
    def test_no_space_left_in_karaoke_room(self):
        guest_4 = Guest("Sandy", 18, 30.50)
        guest_5 = Guest("Kenickie", 21, 55.50)
        guest_6 = Guest("Roger", 16, 35.50)
        guest_7 = Guest("Cha Cha", 25, 10.00)
        self.room_1.check_in_guest_to_room(guest_4)
        self.room_1.check_in_guest_to_room(guest_5)
        self.room_1.check_in_guest_to_room(guest_6)
        self.room_1.check_in_guest_to_room(guest_7)
        self.assertEqual(4, len(self.room_1.guest_list))
        self.assertEqual("No spaces left", self.room_1.room_full(self.room_1.guest_list))
